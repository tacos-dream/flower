...
/**
 * https://github.com/naz013/ColorSlider
 *
...
public class ColorSliderView extends View {
    ...
    private float mSelectorStrokeWidth = 10f;
    private int mDrawCircleRadius = 40;
    ...
    // step数は360固定
    @SuppressWarnings("unused")
    public void setGradient(float saturation) {
        if (saturation >= 0 && saturation <= 1.0) {
            this.calculateColors(saturation, 360);
            this.calculateRectangles();
            this.invalidate();
        }
    }
...
//        this.mSelectorPaint.setStrokeWidth(2f);
        this.mSelectorPaint.setColor(ContextCompat.getColor(context, android.R.color.darker_gray));
        this.mSelectorPaint.setStrokeWidth(mSelectorStrokeWidth);
        ...
        int selectorColor = 0;
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ColorSliderView, 0, 0);
            try {
                selectorColor = a.getColor(R.styleable.ColorSliderView_cs_selector_color, 0);
                int id = a.getResourceId(R.styleable.ColorSliderView_cs_colors, 0);
                int hexId = a.getResourceId(R.styleable.ColorSliderView_cs_hex_colors, 0);
                float saturation = a.getFloat(R.styleable.ColorSliderView_cs_hsv_saturation, -1);   // added by Shinji
                mDrawCircleRadius = a.getInt(R.styleable.ColorSliderView_cs_circle_radius, 40);   // added by Shinji
                mSelectorStrokeWidth = a.getFloat(R.styleable.ColorSliderView_cs_stroke_width, 10);   // added by Shinji
                if (id != 0) {
                    ...
                    // added by Shinji
                    calculateColors(saturation, 360);
                }
                if (this.mColors.length == 0) {
                    int fromColor = a.getColor(R.styleable.ColorSliderView_cs_from_color, 0);
                    int toColor = a.getColor(R.styleable.ColorSliderView_cs_to_color, 0);
                    int steps = a.getInt(R.styleable.ColorSliderView_cs_steps, 21);
                    if (fromColor != 0 && toColor != 0 && steps != 0) {
    ...
    // HSV空間用 added by Shinji
    private void calculateColors(float saturation, int steps) {
        this.mColors = new int[steps];
        final float[] hsv = new float[3];
        hsv[1] = saturation;
        hsv[2] = (float)1.0;           // Brightness
        for (int i = 0; i < steps; i++) {
            hsv[0] = (float)i;
            this.mColors[i] = Color.HSVToColor(hsv);
        }
    }
    ...
    private void drawSlider(Canvas canvas) {
        ...
        if (this.mSelectorPaint != null) {
            for (int i = 0; i < this.mColorRects.length; i++) {
                if (i == this.mSelectedItem) {
                    canvas.drawCircle(
                            this.mColorFullRects[i].left,
                            (this.mColorFullRects[i].top + this.mColorFullRects[i].bottom) / 2,
                            mDrawCircleRadius, mSelectorPaint);
                }
            }
        }
    }
